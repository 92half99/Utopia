...
namespace Utopia {
        ...
        : m_Filepath(path)
    {
        ...
        if (stbi_is_hdr(m_Filepath.c_str()))
        {
            data = reinterpret_cast<uint8_t*>(
                stbi_loadf(m_Filepath.c_str(), &width, &height, &channels, 4)
                );
    ...
    void Image::AllocateMemory(uint64_t size)
    {
        ...
        m_DescriptorSet = reinterpret_cast<VkDescriptorSet>(
    ...
    void Image::Resize(uint32_t width, uint32_t height)
    {
        ...
        constexpr uint32_t MAX_WIDTH = 7680;  // Example maximum width
        constexpr uint32_t MAX_HEIGHT = 4320; // Example maximum height

        // Clamp the width and height to the maximum allowed dimensions
        if (width > MAX_WIDTH || height > MAX_HEIGHT)
            return;

        // If the image is already at the desired size, no action is needed
        ...
        uint64_t newSize = static_cast<uint64_t>(m_Width) * m_Height * Utils::BytesPerPixel(m_Format);

        // Allocate memory and recreate Vulkan resources with the new size
