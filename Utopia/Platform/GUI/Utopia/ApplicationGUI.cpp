...
static void FrameRender(Utopia::Application* application, ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
{
	VkResult err;
...
#include "Utopia/Embed/Utopia-Icon.embed"
#include "Utopia/Embed/WindowImages.embed"
	...
	void Application::Init()
	{
		...
		init_info.DescriptorPool = VK_NULL_HANDLE; // Use DescriptorPoolSize instead for dynamic rendering
		init_info.DescriptorPoolSize = 1000;
#else
		init_info.DescriptorPool = g_DescriptorPool; // Use the existing descriptor pool
		init_info.DescriptorPoolSize = 0;           // Ensure this is zero if DescriptorPool is used
#endif
...
#ifdef USE_DYNAMIC_RENDERING
		init_info.UseDynamicRendering = true; // Set to true if dynamic rendering is enabled
#else
		init_info.UseDynamicRendering = false; // Use the traditional render pass
#endif

		// Ensure RenderPass is set if not using dynamic rendering
		if (!init_info.UseDynamicRendering) {
			init_info.RenderPass = wd->RenderPass;
		}
		else {
			init_info.RenderPass = VK_NULL_HANDLE; // Dynamic rendering doesn't require a render pass
		}
		...
		{
			uint32_t w, h;
			void* data = Image::Decode(g_UtopiaIcon, sizeof(g_UtopiaIcon), w, h);
			m_AppHeaderIcon = std::make_shared<Utopia::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
		{
			uint32_t w, h;
			void* data = Image::Decode(g_WindowMinimizeIcon, sizeof(g_WindowMinimizeIcon), w, h);
			m_IconMinimize = std::make_shared<Utopia::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
		{
			uint32_t w, h;
			void* data = Image::Decode(g_WindowMaximizeIcon, sizeof(g_WindowMaximizeIcon), w, h);
			m_IconMaximize = std::make_shared<Utopia::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
		{
			uint32_t w, h;
			void* data = Image::Decode(g_WindowRestoreIcon, sizeof(g_WindowRestoreIcon), w, h);
			m_IconRestore = std::make_shared<Utopia::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
		{
			uint32_t w, h;
			void* data = Image::Decode(g_WindowCloseIcon, sizeof(g_WindowCloseIcon), w, h);
			m_IconClose = std::make_shared<Utopia::Image>(w, h, ImageFormat::RGBA, data);
			free(data);
		}
	...
	void Application::Shutdown()
	{
		...
		// Release resources
		// NOTE: to avoid doing this manually, we shouldn't
	...
	void Application::UI_DrawTitlebar(float& outTitlebarHeight)
	{
		...
		// Logo
		{
			...
			fgDrawList->AddImage((ImTextureID)(uintptr_t)(m_AppHeaderIcon->GetDescriptorSet()), logoRectStart, logoRectMax);
		}
	...
	{
		if (!m_Specification.UseDockspace)
			UT_CORE_WARN_TAG("Application", "Application::SetMenubarCallback - ApplicationSpecification::UseDockspace is false to menubar will not be visible.");
